...
namespace YS.Admin.CodeGenerator.Template
{
    public class SingleTableTemplate
    {
        #region GetBaseConfig
        public BaseConfigModel GetBaseConfig(string path, string userName, string tableName, string tableDescription, List<TableFieldInfo> tableFieldList)
        {
            ...
            baseConfigModel.OutputConfig.OutputEntity = Path.Combine(path, "YS.Admin.Entity");
            baseConfigModel.OutputConfig.OutputBusiness = Path.Combine(path, "YS.Admin.Business");
            baseConfigModel.OutputConfig.OutputWeb = Path.Combine(path, "YS.Admin.Web", "YS.Admin.Web");
            ...
            baseConfigModel.PageIndex.ColumnList = new List<TableFieldInfo>();
            baseConfigModel.PageIndex.SearchList = new List<TableFieldInfo>();

            baseConfigModel.PageIndex.ColumnList.AddRange(tableFieldList.Take(defaultField));
            baseConfigModel.PageIndex.SearchList.AddRange(tableFieldList.Take(defaultField));
            #endregion

            #region PageFormModel
            baseConfigModel.PageForm = new PageFormModel();
            baseConfigModel.PageForm.ShowMode = 1;
            baseConfigModel.PageForm.FieldList = new List<TableFieldInfo>();
        ...
        public string BuildEntity(BaseConfigModel baseConfigModel, DataTable dt)
        {
            ...
            sb.AppendLine("using YS.Admin.Util;");
            sb.AppendLine();

            sb.AppendLine("namespace YS.Admin.Entity." + baseConfigModel.OutputConfig.OutputModule);
            sb.AppendLine("{");
            ...
            sb.AppendLine("        public string ids { get; set; } = string.Empty;");
            sb.AppendLine("    }");
            sb.AppendLine("}");
        ...
        public string BuildEntityParam(BaseConfigModel baseConfigModel, DataTable dt)
        {
            ...
            sb.AppendLine("namespace YS.Admin.Model.Param." + baseConfigModel.OutputConfig.OutputModule);
            sb.AppendLine("{");
            ...
            #region 添加查询属性
            if (baseConfigModel.PageIndex!=null &&baseConfigModel.PageIndex.IsSearch > 0)
            {
                foreach (var item in baseConfigModel.PageIndex.SearchList)
                {
                    ...
                    {
                        column = dr["TableColumn"].ToString();
                        if (BaseField.BaseFieldList.Where(p => p == column).Any() || column != item.TableColumn)
                        {
                            // 基础字段不需要生成，继承合适的BaseEntity即可。
                            continue;
                        }
        ...
        public string BuildService(BaseConfigModel baseConfigModel, DataTable dt)
        {
            ...
            sb.AppendLine("using YS.Admin.Util;");
            sb.AppendLine("using YS.Admin.Util.Extension;");
            sb.AppendLine("using YS.Admin.Util.Model;");
            sb.AppendLine("using YS.Admin.Data;");
            sb.AppendLine("using YS.Admin.Data.Repository;");
            sb.AppendLine("using YS.Admin.Entity." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YS.Admin.Model.Param." + baseConfigModel.OutputConfig.OutputModule + ";");

            sb.AppendLine();

            sb.AppendLine("namespace YS.Admin.Service." + baseConfigModel.OutputConfig.OutputModule);
            ...
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public async Task SaveForms(List<" + baseConfigModel.FileConfig.EntityName + "> entities)");
            sb.AppendLine("        {");
            sb.AppendLine("            await this.BaseRepository().Update(entities);");
            sb.AppendLine("        }");
            sb.AppendLine();
            ...
            sb.AppendLine("        public async Task<int> Update(Expression<Func<" + baseConfigModel.FileConfig.EntityName + ", bool>> whereLambda, Expression<Func<" + baseConfigModel.FileConfig.EntityName + ", " + baseConfigModel.FileConfig.EntityName + ">> entity)");
            sb.AppendLine("        {");
            sb.AppendLine("            return await this.BaseRepository().Update(whereLambda, entity);");
            sb.AppendLine("        }");
            ...
            if (baseConfigModel.PageIndex!=null&&baseConfigModel.PageIndex.IsSearch > 0)
            {
                foreach (var item in baseConfigModel.PageIndex.SearchList)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        {
                            case "int":
                                sb.AppendLine("                        if (param." + column + "!=null)");
                                sb.AppendLine("                        {");
                                sb.AppendLine("                                expression = expression.And(t => t." + column + "==param." + column + "); ");
                                sb.AppendLine("                        }");
                                break;
                            case "string":
                                sb.AppendLine("                        if (!string.IsNullOrEmpty(param." + column + "))");
                                sb.AppendLine("                        {");
                                sb.AppendLine("                                expression = expression.And(t => t." + column + ".Contains(param." + column + ")); ");
                                sb.AppendLine("                        }");
                                break;
                            case "long?":
                                sb.AppendLine("                        if (param." + column + "!=null)");
                                sb.AppendLine("                        {");
                                sb.AppendLine("                                expression = expression.And(t => t." + column + " == param." + column + ".ParseToLong());");
                                sb.AppendLine("                        }");
                                break;
                            case "DateTime?":
                                sb.AppendLine("                        if (param." + column + "!=null)");
        ...
        public string BuildBusiness(BaseConfigModel baseConfigModel)
        {
            ...
            sb.AppendLine("using YS.Admin.Entity." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YS.Admin.Model.Param." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YS.Admin.Service." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using System.Linq.Expressions;");
            sb.AppendLine();

            sb.AppendLine("namespace YS.Admin.Business." + baseConfigModel.OutputConfig.OutputModule);
            sb.AppendLine("{");
            ...
            sb.AppendLine("            obj.Data = entity.Id.ToString();");
            sb.AppendLine("            obj.Tag = 1;");
            sb.AppendLine("            return obj;");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public async Task<TData<string>> SaveForms(List<" + baseConfigModel.FileConfig.EntityName + "> entities)");
            sb.AppendLine("        {");
            sb.AppendLine("            TData<string> obj = new TData<string>();");
            sb.AppendLine("            await " + TableMappingHelper.FirstLetterLowercase(baseConfigModel.FileConfig.ServiceName) + ".SaveForms(entities);");
            sb.AppendLine("            obj.Tag = 1;");
            sb.AppendLine("            return obj;");
            sb.AppendLine("        }");
            sb.AppendLine();
            ...
            sb.AppendLine("        public async Task<TData<string>> UpdateForms(" + baseConfigModel.FileConfig.EntityName + " entity, Expression<Func<" + baseConfigModel.FileConfig.EntityName + ", " + baseConfigModel.FileConfig.EntityName + ">> updateExpression)");
            sb.AppendLine("        {");
            sb.AppendLine("            TData<string> obj = new TData<string>();");
            sb.AppendLine("            long[] IdsList = TextHelper.SplitToArray<long>(entity.ids, ',');");
            sb.AppendLine("            if (IdsList.Length.Equals(0))");
            sb.AppendLine("            {");
            sb.AppendLine("             	obj.Tag = 0;");
            sb.AppendLine("                 obj.Message = \"请选择操作数据\";");
            sb.AppendLine("                 return obj;");
            sb.AppendLine("            }");
            sb.AppendLine("            ");
            sb.AppendLine("            await " + TableMappingHelper.FirstLetterLowercase(baseConfigModel.FileConfig.ServiceName) + ".Update(t => IdsList.Contains(t.Id.Value), updateExpression);");
            sb.AppendLine("            obj.Tag = 1;");
            sb.AppendLine("            return obj;");
            sb.AppendLine("        }");
        ...
        public string BuildController(BaseConfigModel baseConfigModel)
        {
            ...
            sb.AppendLine("using YS.Admin.Util;");
            sb.AppendLine("using YS.Admin.Util.Model;");
            sb.AppendLine("using YS.Admin.Entity;");
            sb.AppendLine("using YS.Admin.Model;");
            sb.AppendLine("using YS.Admin.Web.Controllers;");
            sb.AppendLine("using YS.Admin.Entity." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YS.Admin.Business." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine("using YS.Admin.Model.Param." + baseConfigModel.OutputConfig.OutputModule + ";");
            sb.AppendLine();

            sb.AppendLine("namespace YS.Admin.Web.Areas." + baseConfigModel.OutputConfig.OutputModule + ".Controllers");
            sb.AppendLine("{");
        ...
        public string BuildIndex(BaseConfigModel baseConfigModel)
        {
            ...
            if (baseConfigModel.PageIndex.SearchList == null)
            {
                baseConfigModel.PageIndex.SearchList = new List<TableFieldInfo>();
            }
            #endregion
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("@{");
            sb.AppendLine("    Layout = \"~/Areas/System/Views/Shared/_Index.cshtml\";");
            sb.AppendLine(" }");
            ...
            if (baseConfigModel.PageIndex.IsSearch == 1)
            {
                ...
                foreach (var item in baseConfigModel.PageIndex.SearchList)
                {
                    string fieldName = item.TableColumn;
                    string fieldNameLower = TableMappingHelper.FirstLetterLowercase(item.TableColumn);
                    string reamrk = string.IsNullOrEmpty(item.Remark) ? fieldName : item.Remark;

                    sb.AppendLine("                        " + reamrk + "：<input id=\"" + fieldNameLower + "\" col=\"" + fieldName + "\" type=\"text\" />");
            ...
            {
                string reamrk = string.IsNullOrEmpty(item.Remark) ? item.TableColumn : item.Remark;
                sb.AppendLine("                { field: '" + item.TableColumn + "', title: '" + reamrk + "' },");
            }

            sb.AppendLine(" {");
            sb.AppendLine("    title: '操作',");
            sb.AppendLine("    formatter: function (value, row, index) {");
            sb.AppendLine("        var actions = [];");
            sb.AppendLine("        actions.push('<a class=\"btn btn-success btn-xs \" href=\"javascript:void(0)\" onclick=\"showEdit(\\'' + row.Id + '\\')\"><i class=\"fa fa-edit\"></i>编辑</a> ');");
            sb.AppendLine("        return actions.join('');");
            sb.AppendLine(" }");
            sb.AppendLine(" }");
            ...
            if (baseConfigModel.PageIndex.ButtonList.Where(p => p == "btnAdd" || p == "btnEdit").Any())
            {
                ...
                sb.AppendLine("  showEdit(id)");
                sb.AppendLine("    }");



                sb.AppendLine("     function showEdit(id) {");
        ...
        public string BuildForm(BaseConfigModel baseConfigModel)
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("@{");
            sb.AppendLine("    Layout = \"~/Areas/System/Views/Shared/_FormWhite.cshtml\";");
            sb.AppendLine("}");
            sb.AppendLine("");
            ...
            if (baseConfigModel.PageForm.FieldList.Count > 0)
            {
                ...
                switch (baseConfigModel.PageForm.ShowMode)
                {
                    case 1:
                        ...
                        {
                            field = baseConfigModel.PageForm.FieldList[i].TableColumn;
                            remark = string.IsNullOrEmpty(baseConfigModel.PageForm.FieldList[i].Remark) ? field : baseConfigModel.PageForm.FieldList[i].Remark;
                            fieldLower = TableMappingHelper.FirstLetterLowercase(field);

                            sb.AppendLine("        <div class=\"form-group\">");
                            sb.AppendLine("            <label class=\"col-sm-3 control-label \">" + remark + (i == 0 ? "" : string.Empty) + "</label>");
                    ...
                    case 2:
                        for (int i = 0; i < baseConfigModel.PageForm.FieldList.Count; i++)
                        {
                            ...
                            sb.AppendLine("            <label class=\"col-sm-2 control-label \">" + remark + "</label>");
        ...
        public async Task<List<KeyValue>> CreateCode(BaseConfigModel baseConfigModel, string code)
        {
            ...
            #region 实体类
            if (!string.IsNullOrEmpty(param["CodeEntity"].ToString()))
            {
                string codeEntity = HttpUtility.HtmlDecode(param["CodeEntity"].ToString());
                string codePath = Path.Combine(baseConfigModel.OutputConfig.OutputEntity, "YS.Admin.Entity", baseConfigModel.OutputConfig.OutputModule, baseConfigModel.FileConfig.EntityName + ".cs");
            ...
            if (!param["CodeEntityParam"].IsEmpty())
            {
                ...
                string codePath = Path.Combine(baseConfigModel.OutputConfig.OutputEntity, "YS.Admin.Model", "Param", baseConfigModel.OutputConfig.OutputModule, baseConfigModel.FileConfig.EntityParamName + ".cs");
                if (!File.Exists(codePath))
                {
            ...
            if (!param["CodeService"].IsEmpty())
            {
                ...
                string codePath = Path.Combine(baseConfigModel.OutputConfig.OutputBusiness, "YS.Admin.Service", baseConfigModel.OutputConfig.OutputModule, baseConfigModel.FileConfig.ServiceName + ".cs");
                if (!File.Exists(codePath))
                {
            ...
            if (!param["CodeBusiness"].IsEmpty())
            {
                ...
                string codePath = Path.Combine(baseConfigModel.OutputConfig.OutputBusiness, "YS.Admin.Business", baseConfigModel.OutputConfig.OutputModule, baseConfigModel.FileConfig.BusinessName + ".cs");
                if (!File.Exists(codePath))
                {
