...
namespace YS.Admin.Data.EF
{
    public class SqlServerDatabase : IDatabase
    {
        ...
        public async Task<T> Insert2<T>(T entity) where T : class
        {
            dbContext.Entry<T>(entity).State = EntityState.Added;

            int m = await dbContext.SaveChangesAsync();
            //int n = dbContextTransaction == null ? await this.CommitTrans() : 0;
            if (m > 0)
            {
                return entity;
            }
		...
		public async Task<int> Update<T>(Expression<Func<T, bool>> condition, Expression<Func<T, T>> updateExpression) where T : class, new()
		{
			...
			var entitiesToUpdate = dbContext.Set<T>().Where(condition);
			...
			int updatedCount = await dbContext.SaveChangesAsync();
			return updatedCount;
