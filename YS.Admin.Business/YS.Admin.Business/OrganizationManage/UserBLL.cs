...
namespace YS.Admin.Business.OrganizationManage
{
    public class UserBLL
    {
        ...
        public async Task<TData<List<UserEntity>>> GetPageList(UserListParam param, Pagination pagination)
        {
            ...
            List<UserRoleInfo> userRoleInfos = await roleService.GetUserRoleList(null);
            ...
            List<UserBelongEntity> userBelongList = await userBelongService.GetList(new UserBelongEntity { UserIds = obj.Data.Select(p => p.Id.Value).ToStrings<long>() });
            List<DepartmentEntity> departmentList = await departmentService.GetList(new DepartmentListParam { Ids = userBelongList.Select(p => p.BelongId.Value).ToStrings<long>() });
            foreach (UserEntity user in obj.Data)
            {
                ...
                List<UserRoleInfo> userRoles = userRoleInfos.Where(p => p.UserId.Equals(user.Id)).ToList();
                if (userRoles.Count > 0)
                {
                    user.RoleName = string.Join(',', userRoles.Select(p => p.RoleName).ToArray());
                }
            }
        ...
        /// <summary>
        /// 解除界面锁定 - 由于 锁定未暂离开状态，不按照 加密的方式
        /// </summary>
        /// <param name="entity"></param>
        /// <returns></returns>
        public async Task<TData<long>> LockJson()
        {
            TData<long> obj = new TData<long>();

            UserEntity entity = await userService.LockJson();
            obj.Message = "锁定成功";
            obj.Tag = 1;
            await RemoveCacheById(entity.Id.Value);
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public async Task<TData<long>> UnLockJson(string lockpwd)
        {
            TData<long> obj = new TData<long>();

            bool status = await userService.UnLockJson(lockpwd);
            if (status)
            {
                obj.Message = "解锁成功";
                obj.Tag = 1;
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public async Task<TData<long>> UnLockByIdJson(string Id)
        {
            TData<long> obj = new TData<long>();

            bool status = await userService.UnLockByIdJson(Id);
            if (status)
            {
                obj.Message = "解锁成功";
                obj.Tag = 1;
        ...
        private async Task RestCacheById(long id)
        {
            var dbEntity = await userService.GetEntity(id);
            if (dbEntity != null)
            {
