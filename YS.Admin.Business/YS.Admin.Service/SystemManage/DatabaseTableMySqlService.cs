...
namespace YS.Admin.Service.SystemManage
{
    public class DatabaseTableMySqlService : RepositoryFactory, IDatabaseTableService
    {
		...
		/// <summary>
		/// 仅用在框架里面，将当前链接的数据库 同步到另外一个链接中，以 MySql 为主库，同步 MySql 的数据到SqlServer和Oracle，保证各个数据库的数据是一样的
		/// </summary>
		/// <returns></returns>
		public async Task SyncDatabase()
        {
            #region 同步SqlServer数据库  这里创建对应的表实体
        ...
        {
            string sqlServerConnectionString = "Server=localhost;Database=YS.AdminAdmin;User Id=sa;Password=123456;";
