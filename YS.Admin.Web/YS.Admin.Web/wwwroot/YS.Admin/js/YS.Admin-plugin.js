...
; (function ($) {
        ...
        var dom = {
            ...
            render: function (setting) {
                if (setting.data && setting.data.length >= 0) {
                    ...
                    var html = "";
                    var alldisabled = "";
                    if (!ys.isNullOrEmpty(setting.alldisabled)) {
                    ...
                    $.each(setting.data, function (i) {
                        ...
                        var disabled = "";
                        if (!ys.isNullOrEmpty(setting.disabled)) {
                            if (row[setting.key] == setting.disabled) {
                                disabled = "disabled";
                            }
                        }
                        var strclick = "";
                        // 为每个单选框增加click事件处理
                        if (setting.onClick) {
                            strclick = ' onclick="cl' + name + '(\'' + row[setting.key] + '\')"';
                        }


                        html += "<label class='radio-box' " + strclick +">";
                        html += "<input type='radio' name='" + name + "' " + disabled + " " + alldisabled + "  value='" + row[setting.key] + "' ref='" + ref + "' /> " + row[setting.value];
                        html += "</label>";
            ...
            render: function (setting) {
                if (setting.data && setting.data.length >= 0) {
                    ...
                    $.each(setting.data, function (i) {
                        ...
                        html += "<input name='" + name + "' type='checkbox' " + disabled + " value='" + row[setting.key] + "'>" + row[setting.value] + "</input>";
                        html += "</label>";
    ...
    $.fn.ysCheckBox.methods = {
        ...
        setValue: function (target, value) {
            ...
            var ids = value.split(',');
            $.each(ids, function (i, val) {
                var checkbox = $(target).find('input[type=checkbox][class!="otherSel"][value=' + val + ']');
        ...
        var dom = {
            ...
            render: function (setting) {
                if (setting.data && setting.data.length >= 0) {
                    ...
                    var html = "<select id='" + id + "' name='" + id + "' " + alldisabled + " class='" + ys.toString(setting.class) + " select2' " + mutiple + " maxheight=" + setting.maxHeight + ">";
                    ...
                    if (!groupOption) {
                        if (setting.noall != 1) {
                            if (!setting.class) {
                                if (setting.choiceType) {
                                    option += "<option value='-1'>" + setting.choiceType + "</option>";
                                }
                                else {
