...
function selection(value) {
    ...
    //$("#answer").val(value);
    var value = '';
    $('input[name="checkbox-p-1"]:checked').each(function () {
        value += ($(this).attr("v-val"));
    });
    $("#answer").val(value);
}
...
function appeddata(index, st) {
    ...
    $("#fields").append('<tr><td>&nbsp;<div class="exam-radio"><input onclick="selection(\'' + arLetter[index] + '\')" v-val="' + arLetter[index] + '" type="checkbox" name="checkbox-p-1" id="f' + index + '" ' + checkstr + ' ><label for="f' + index + '"  class="cr">' + st + '</label><input type="hidden" name="timulist" value="' + st + '"></div></td></tr>');
}
