layui.use(['jquery', 'flow'], function () {
    ...
    flow.load({
        elem: "#LAY_bloglist",
            ...
            $.ajax({
                type: "POST",
                url: '/BlogWebsite/Article/LoadArticleByClass',
                data: {
                    classId: type,
                    page: page,
                    pagesize: pagesize
    ...
    $("#searchtxt").on("keyup", function () {
        setTimeout(function () {
            ...
            "" == $("#searchtxt").val().trim() ? $(".search-result").empty().hide() : $.ajax({
                type: "post",
                url: '/BlogWebsite/Article/SearchResult',
                data: {
                ...
                success: function (a) {
                    "[]" != a ? ($(".search-result").show().empty(), $.each(a,
                        function (t, i) {
                            $(".search-result").append('<li class="child"><a href="/BlogWebsite/Article/Detail/' + i.Id + '" style="display:block" target="_blank">' + i.Title.toLowerCase().replace($("#searchtxt").val().trim().toLowerCase(), '<b style="color:#6bc30d">' + $("#searchtxt").val().trim() + "<\/b>") + "<\/a><\/li>")
